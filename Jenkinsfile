...
pipeline{
  ...
  stages {
    ...
    stage('regressions-11.1-Latest') {
      ...
      steps {
        runAllTests('Latest', '11.1', false)
        junit '**/build/**/TEST*.xml'
      }
    }

    stage('regressions-11.1-Latest-reverseProxy') {
    	when {
				allOf {
					branch 'develop'
					expression {return params.regressions}
