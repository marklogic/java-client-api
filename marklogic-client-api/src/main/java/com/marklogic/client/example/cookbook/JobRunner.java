...
public long getLastFailureLogTime() { // change
...
public void run(DatabaseClient client, InputStream csvRecords, InputStream jobControl) throws IOException {
      ...
      final class JobRunnerVariables{
    	  ...
    	  // synchronized method for successCount and failure
      }
      ...
      try {
  		...
  		JsonNode metadataNode = jobDef.path("metatadata");
                     ...
                     batch -> {
                    	 if(getBatchSize()==1) {
                    		 ...
                    		 if(jobRunnerVariables.totalSuccessCount == 0){
                    			 ...
                    			 // jobRunnerVariables.lastSuccessCount.set(jobRunnerVariables.totalSuccessCount.longvalue)
                    			 // jobRunnerVariables.lastSuccessCount.incrementandGet()
                    		 } 
...
// write after job document with job metadata in /jobEnd and /jobs/ID collections or send after job payload to DHF endpoint
    	...
    	//  logger.info("Finishing the job. Time elapsed = " + (System.currentTimeMillis() - startTime)/1000 + "seconds.");
    	  System.out.println("Finishing the job. Time elapsed = " + (System.currentTimeMillis() - startTime)/1000 + " seconds.");
    	  moveMgr.release();
      }
   }
