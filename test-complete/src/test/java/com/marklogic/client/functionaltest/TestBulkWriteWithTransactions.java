...
public class TestBulkWriteWithTransactions extends BasicJavaClientREST {
	...
	public void validateMetadata(DocumentMetadataHandle mh) {
		...
		// Permissions
		String[] expectedPermissions1 = "size:4|flexrep-eval:[READ]|rest-reader:[READ]|app-user:[UPDATE, READ]|rest-writer:[UPDATE]|".split("\\|");		 
		String actualPermissions = getDocumentPermissionsString(permissions);
		//System.out.println(actualPermissions);
		
		boolean bPermissionsFound = false;
		List<String> retdPermissions = new ArrayList<String>(Arrays.asList(actualPermissions.split("\\|")));
		for(String str : expectedPermissions1) {
			if (str.startsWith("app")) {
				if (retdPermissions.contains("app-user:[UPDATE, READ]") || retdPermissions.contains("app-user:[READ, UPDATE]"))
					{ bPermissionsFound = true; }
				else { bPermissionsFound = false; break;}
				// Skip rest of check in the present iteration when true and continue to next element.
				continue;
			}
				
			if (retdPermissions.contains(str))
				{ bPermissionsFound = true; }
			else { bPermissionsFound = false; break;}				
		}		
		assertTrue("Document permissions difference", bPermissionsFound);

		// Collections 
		String[] expectedCollections = "size:2|my-collection1|my-collection2|".split("\\|");
		String actualCollections = getDocumentCollectionsString(collections);
		
		boolean bCollectionsFound = false;
		List<String> retdcollections = new ArrayList<String>(Arrays.asList(actualCollections.split("\\|")));
		for(String str : expectedCollections) {
			if (retdcollections.contains(str))
				{ bCollectionsFound = true; }
			else { bCollectionsFound = false; break;}				
		}		
		assertTrue("Document collections difference", bCollectionsFound);
	}
	...
	public void validateDefaultMetadata(DocumentMetadataHandle mh) {
		...
		// Permissions
		String[] expectedPermissions1 = "size:3|flexrep-eval:[READ]|rest-reader:[READ]|rest-writer:[UPDATE]|".split("\\|");		 
		String actualPermissions = getDocumentPermissionsString(permissions);
		System.out.println(actualPermissions);
