 ...
 *        .withBinding("o", "http://example.org/o1");
 ...
 * {@link DatabaseClient#newSPARQLQueryManager}.  While this example uses
 * JacksonHandle, any SPARQLResultsReadHandle may be used--including custom
 * handles.  While {@link JSONReadHandle}s will need to use SPARQL_JSON mimetype,
 * and {@link XMLReadHandle}s will need to use SPARQL_XML mimetype,
 * most {@link TriplesReadHandle}s accept any text and can therefore accept
 * results in any of the RDFMimeTypes or SPARQLMimeTypes.
 *
 * <p>SPARQLQueryManager is
 * thread-safe other than setPageLength. In other words the only state
 * maintained by an instance is the page length.  Common usage is to call
 * setPageLength only once then use the instance across many threads.  If you
 * intend to call setPageLength from multiple threads, create a new
 * SPARQLQueryManager for each thread.</p>
 *
